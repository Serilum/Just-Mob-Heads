...
public class HeadDropEvent {	
	public static void mobItemDrop(Level level, Entity entity, DamageSource damageSource) {
		...
		if (ConfigHandler.enableLootingEnchant && source instanceof Player) {
			int looting = EnchantmentHelper.getEnchantmentLevel(level.registryAccess().lookupOrThrow(Registries.ENCHANTMENT).getOrThrow(Enchantments.LOOTING), (LivingEntity)source);
			if (looting > 0) {
		...
		if (itemStack.has(DataComponents.CUSTOM_NAME)) {
			if (!itemStack.has(DataComponents.NOTE_BLOCK_SOUND)) {
				...
				if (HeadData.headNoteBlockSounds.containsKey(dataHeadName)) {
					itemStack.set(DataComponents.NOTE_BLOCK_SOUND, Identifier.parse(HeadData.headNoteBlockSounds.get(dataHeadName)));
				}
			}
